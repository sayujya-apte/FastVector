...
template <typename T> class FastVector
{
  ...
  private:
    ...
    static void deallocate(T* ptr, size_t n)
    {
        ...
        // calling ::operator delete() instead of delete() forces a call to global operator and not an overriden
        // operator
    ...
    void grow()
    {
        ...
        if constexpr (std::is_nothrow_move_constructible_v<T>)
        {
            for (size_t i = 0; i < m_Size; i++)
            {
                ...
                //     * normal new operator does two things - allocates memory, and constructs object in allocated
                //     memory
                //     * placement new operator allows us to separate those two things
                //     * new (address) dtype (initializer)
                //     * because allocate() returns raw uninitialized memory, it is necessary to use placement new to
                //     construct the object inside that memory
